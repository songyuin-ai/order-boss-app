...
// 날짜별 보기는 마감된 과거 기간 + 진행 중인 현재 기간(이번 주/이번 달)까지 조회 가능.
// (v5) POS 데이터는 전일자 마감 데이터까지만 제공되어 "오늘" 하루 단위 조회 자체가 불가능하므로
// 일간 탭에서는 "오늘"을 제거하고 가장 최근 마감일(전일)부터 시작함
// 주간·월간 탭의 가장 최근 항목(index 0, 이번 주/이번 달)은 아직 마감되지 않은 진행 중 기간이라
// 전주/전월 대비 배지가 없음 — 다만 이 값도 전일자 마감 데이터까지 반영된 수치라 화면에 안내 문구 표시
