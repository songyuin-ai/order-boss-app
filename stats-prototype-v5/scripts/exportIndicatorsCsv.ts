...
import { homeIndicators, homeHookCardIndicatorPending } from "../src/data/homeIndicators";
...
rows.forEach((r) => {
  lines.push(
    [
      ...
      r.제공목적,
      r.차트형태,
      r.id === "data_028" ? r.참고 ?? "" : "", // 참고 — data_028(세그먼트 정의, 손님 세그먼트 비율)만 기존 내용 유지, 나머지는 비움
    ]
      .map(csvCell)
      .join(",")
  );
